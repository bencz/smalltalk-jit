// ppc64le (little-endian, ELFv2) backend, the code generator. A deliberate
// COPY of vm/jit/ppc64/CodeGeneratorPpc64.c: same ISA, same register/frame
// mapping, and the ONLY divergence is the ELFv2 C-call frame (32-byte header
// with the TOC save at 24(r1), and no parameter save area). Read
// vm/jit/ppc64/DESIGN.md first for the mapping and the bring-up war stories,
// then vm/jit/ppc64le/DESIGN.md for the deltas.
//
// Key invariants, identical to the BE backend: frame layout byte-compatible
// with the x64 one (StackFrame.c walker untouched),
// FP=r31/CTX=r30/TGT=r12/TMP=r11/TMP2=r10/result=r3, LR discipline (bodies: LR
// dead after the prologue pushed it; generateCCall PUSHES LR before FP so
// [FP+8] is the IC the GC's frame walker reads; framed primitives push LR
// first), explicit little-endian emission, and tagged-base ld/std through
// asmLdT/StdT.
...
static void generateSafepointPoll(CodeGenerator *generator, _Bool atBackEdge)
{
	...
	abiEmitCallerSavedPush(&AbiPpc64leElfV2, buffer);
	...
	abiEmitCallerSavedPop(&AbiPpc64leElfV2, buffer);
...
// on ISA 2.07+ (always true on this LE backend, baseline POWER8), else the
// per-thread TLS scratch. The result lands in r3 on every committed path.
...
// C call from JIT code (ELFv2). The stack shape must mirror x64 EXACTLY,
...
void generateCCall(CodeGenerator *generator, intptr_t cFunction, size_t argsSize, _Bool storeIp)
{
	...
	// ELFv2 callee frame: just the 32-byte header, where the callee stores its
	// LR/CR in OUR slots at 16/8(r1) and 24(r1) is the TOC save slot
	// emitCallCFunction uses. NO parameter save area: it is optional under
	// ELFv2 and our C callees are prototyped, non-variadic and take at most 8
	// arguments, so gcc itself frames even an 8-argument indirect call with
	// exactly this `stdu 1,-32(1)` (verified against the toolchain, see
	// vm/jit/ppc64le/DESIGN.md item 2). ELFv1 needed 112 = 48 + 64.
	asmStdu(buffer, R1, -32, R1);
	...
	gPpc64leAbi->emitCallCFunction(buffer, cFunction);
