...
// Serialize the graph rooted at `root` into a freshly malloc'd buffer. Sets
// *outSize and returns the buffer (caller frees). Returns NULL if the graph
// contains a non-portable object (block/context/compiled code).
uint8_t *messageSerialize(RawObject *root, size_t *outSize);

// Rebuild the graph from `bytes` into CurrentThread's heap. Returns the tagged
// root Value, or 0 on error. May allocate (and thus GC) — callers holding other
// live roots must have them in handles.
